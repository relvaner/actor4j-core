	...
	public void test_restart() {
		...
		UUID parent = system.addActor(() -> new Actor("parent") {
			...
			public void preStart() {	
				addChild(() -> new Actor("child") {
					...
					public void postStop() {
						...
						if (i==1 || i==2) {
							logger().log(DEBUG, "child::postStop: "+i);
							testDone.countDown();
						}
					}
			...
			public void postStop() {
				int i = counter.incrementAndGet();
				if (i==3) {
					logger().log(DEBUG, "parent::postStop: "+i);
					testDone.countDown();
				}
			}
			...
			public void preRestart(Exception reason) {
				...
				int i = counter.incrementAndGet();
				if (i==1 || i==2) {
					logger().log(DEBUG, "parent::preRestart: "+i);
					testDone.countDown();
				}
			}
			...
			public void postRestart(Exception reason) {
				...
				int i = counter.incrementAndGet();
				assertEquals(4, i);
				logger().log(DEBUG, "parent::postRestart: "+i);
				testDone.countDown();
			}
		...
		UUID parent = system.addActor(() -> new Actor("parent") {
			...
			public void preStart() {
				...
				addChild(() -> new Actor("child1") {
					@Override
					public void postStop() {
						logger().log(DEBUG, "child1::postStop::"+counter.get());
						if (counter.get()==1)
							assertEquals(2, counter.incrementAndGet());
						testDone.countDown();
					}
					...
					@Override
					public void preRestart(Exception reason) {
						logger().log(DEBUG, "child1::preRestart::"+counter.get());
						assertEquals(1, counter.incrementAndGet());
						super.preRestart(reason);
						testDone.countDown();
					}
					...
					@Override
					public void postRestart(Exception reason) {
						logger().log(DEBUG, "child1::postRestart::"+counter.get());
						assertEquals(3, counter.incrementAndGet());
						super.postRestart(reason);
						testDone.countDown();
					}
				...
				child2 = addChild(() -> new Actor("child2") {
					@Override
					public void postStop() {
						logger().log(DEBUG, "child2::postStop");
						testDone.countDown();
					}
					...
					public void preRestart(Exception reason) {
						super.preRestart(reason);
						logger().log(DEBUG, "child2::preRestart");
						testDone.countDown();
					}
					...
					public void postRestart(Exception reason) {
						super.postRestart(reason);
						logger().log(DEBUG, "child2::postRestart");
						testDone.countDown();
					}
				...
				addChild(() -> new Actor("child3") {
					@Override
					public void postStop() {
						logger().log(DEBUG, "child3::postStop");
						testDone.countDown();
					}
					...
					public void preRestart(Exception reason) {
						super.preRestart(reason);
						logger().log(DEBUG, "child3::preRestart");
						testDone.countDown();
					}
					...
					public void postRestart(Exception reason) {
						super.postRestart(reason);
						logger().log(DEBUG, "child3::postRestart");
						testDone.countDown();
					}
